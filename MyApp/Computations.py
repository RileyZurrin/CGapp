...
def p1(J,j1,j2):
    return ((2*J + 1)*factorial(J + j1 - j2, exact = True)*factorial(J - j1 + j2, exact = True)*factorial(j1 + j2 - J, exact = True))/(factorial(j1 + j2 + J +1, exact = True))

# second multipliers (top and bottom)
def ptop(J,M,j1,m1,j2,m2):
    return factorial(J + M, exact = True)*factorial(J - M, exact = True)*factorial(j1 - m1, exact = True)*factorial(j1 + m1, exact = True)*factorial(j2 - m2, exact = True)*factorial(j2 + m2, exact = True)

def pbot(J,j1,m1,j2,m2):
    return factorial(J + m1 + m2, exact = True)*factorial(J - (m1+m2), exact = True)*factorial(j1 - m1, exact = True)*factorial(j1 + m1, exact = True)*factorial(j2 - m2, exact = True)*factorial(j2 + m2, exact = True)


# third multiplier
def summand(k,j1,j2,J,m1,m2):
    return ((-1)**k)/(factorial(k, exact = True)*factorial(j1 + j2 - J - k, exact = True)*factorial(j1 - m1 - k, exact = True)*factorial(j2 + m2 - k, exact = True)*factorial(J - j2 + m1 + k, exact = True)*factorial(J - j1 - m2 + k, exact = True))
